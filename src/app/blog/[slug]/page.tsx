  ...
  return (
    <>
      ...
      <main className="flex-grow bg-[#fbf8f1]">
        <article>
          <header className="relative min-h-[520px] overflow-hidden bg-slate-950 text-white sm:min-h-[720px]">
            ...
            <div className="relative mx-auto flex min-h-[520px] max-w-7xl flex-col justify-end px-4 pb-8 pt-20 sm:min-h-[720px] sm:px-6 sm:pb-20 lg:px-8">
              <Link href="/blog" className="absolute top-5 inline-flex items-center gap-2 rounded-full border border-white/25 bg-slate-950/20 px-3 py-1.5 text-xs font-bold text-white backdrop-blur-md transition hover:bg-white hover:text-slate-950 sm:top-10 sm:px-4 sm:py-2 sm:text-sm"><ArrowLeft className="h-4 w-4" />Tous les articles</Link>
              <div className="max-w-4xl">
                <p className="inline-flex items-center gap-2 rounded-full bg-orange-600 px-3 py-1.5 text-[0.65rem] font-black uppercase tracking-[0.14em] sm:px-4 sm:py-2 sm:text-xs"><Mountain className="h-3.5 w-3.5" />{post.category}</p>
                <h1 className="mt-4 font-display text-3xl font-black leading-[1.04] tracking-tight sm:mt-6 sm:text-6xl lg:text-7xl">{post.title}</h1>
                <p className="mt-3 line-clamp-3 max-w-3xl text-sm leading-6 text-slate-200 sm:mt-6 sm:block sm:text-xl sm:leading-8">{post.excerpt}</p>
                <div className="mt-4 flex flex-wrap items-center gap-x-4 gap-y-2 border-t border-white/20 pt-4 text-xs font-semibold text-slate-300 sm:mt-8 sm:gap-x-6 sm:gap-y-3 sm:pt-6 sm:text-sm">
                  ...
                  <span className="hidden items-center gap-2 sm:inline-flex"><Compass className="h-4 w-4 text-orange-400" />Conseils de guides locaux</span>
                </div>
              </div>
            </div>
          </header>

          <div className="mx-auto grid max-w-7xl gap-12 px-4 py-10 sm:px-6 sm:py-24 lg:grid-cols-[250px_minmax(0,720px)] lg:justify-center lg:gap-16 lg:px-8">
